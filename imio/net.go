...
func RegisterHttpListener() {
	...
	http.Handle("/user/roomBg", netHandler(changeCustomRoomBg))
	http.Handle("/friend/notify", netHandler(friendMsgNotify))
	http.Handle("/file/emo", netHandler(getEmo))
	http.Handle("/friend/background", netHandler(changeCustomFriendBg))
	http.Handle("/friend/markName", netHandler(upDateFriendMarkName))
	http.Handle("/room/markName", netHandler(upDateRoomMarkName))
	http.Handle("/user/netAddress", netHandler(getUserNetAddress))
	StartWebsocket()
